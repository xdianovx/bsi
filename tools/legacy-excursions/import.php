...
/**
 * CLI-импорт экскурсий старого сайта (локальная разработка).
 *
 * На проде доступа к CLI нет — там используется страница
 * «Инструменты → Импорт экскурсий» (inc/admin/legacy-excursions-import.php).
 ...
 */
function bsi_legacy_cli_input(): array
{
...
function bsi_legacy_cli_run(array $input): void
{
  ...
  $loaded = bsi_legacy_load_payload($input['path']);
  ...
  $stats = bsi_legacy_import_items($items, $loaded['country_id'], $input['status'], $input['dry_run']);
  ...
  printf(
    "\nСоздано: %d, обновлено: %d, пропущено: %d, конфликтов: %d, с ценами: %d\n",
    $stats['created'],
    $stats['updated'],
    $stats['skipped'],
    $stats['conflicts'],
...
$bsi_legacy_cli_input = bsi_legacy_cli_input();

define('WP_USE_THEMES', false);
require $bsi_legacy_cli_input['wp_load'];

bsi_legacy_cli_run($bsi_legacy_cli_input);
