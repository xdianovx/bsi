 ...
 *
 * 0 — без ограничения: выводим всё, что выбрано в админке. Раньше здесь был
 * лимит 12 (60 у туров) ради веса HTML, но из-за него новые карточки,
 * добавленные в подборку, просто не появлялись на фронте.
 * Картинки карточек вне первых слайдов грузятся лениво (см. tour/card.php),
 * поэтому вес страницы растёт умеренно.
 *
 * Вернуть ограничение можно фильтром bsi_homepage_slider_limit.
 *
 * @param string $context Слайдер: 'tour', 'education', 'hotel', 'event' или ''.
 * @return int Максимум карточек; 0 — без лимита.
 */
function bsi_homepage_slider_limit(string $context = ''): int
{
	$limit = (int) apply_filters('bsi_homepage_slider_limit', 0, $context);
