...
function bsi_ajax_popular_education_by_country()
{
  ...
  if ($q->have_posts()) {
    while ($q->have_posts()) {
      ...
      // Цена: та же логика, что single-education.php и popular.php (программы → helper → legacy),
      // с конвертацией валюты в рубли. Иначе сырое program_price_per_week завышает цену.
      $price = '';
      if (function_exists('get_field')) {
