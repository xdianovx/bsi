 ...
 * ВАЖНО: этот код должен выполниться ДО init(20), где ты регистрируешь region/resort в country CPT файле.
 */
...
add_action('init', function () {
  ...
  register_post_type('tour', [
    ...
    // галерея будет через ACF, thumbnail можно оставить для превью/карточек
    'supports' => ['title', 'editor', 'thumbnail', 'excerpt'],

    // чтобы метабоксы таксономий точно были в редакторе
...
/**
 * 3) Таксономия типов туров (экскурсионные и т.д.)
 */
add_action('init', function () {
...
/**
 * 4) На всякий: принудительно привяжем region/resort к tour (даже если фильтр не успел)
 * Это спасает 99% кейсов “в админке не вижу регионы/курорты”.
 */
add_action('init', function () {
...
/**
 * 6) Роут: /country/{country}/tours/{tour}/ -> single tour
 */
add_action('init', function () {
  add_rewrite_rule(
    '^country/([^/]+)/tours/([^/]+)/?$',
...
/**
 * 8) Валидация пути: если страна в URL не совпадает — 404
 */
...
/**
 * 9) ACF — отдельная группа “Страна” под заголовком
 */
add_action('acf/init', function () {
  ...
  /**
   * 10) ACF — остальное (ниже), без seamless
   */
  acf_add_local_field_group([
...
add_filter('wpseo_breadcrumb_links', function ($links) {
  ...
  if (is_singular('tour')) {
    ...
    if ($countries_url) {
      $new_links[] = ['url' => $countries_url, 'text' => $countries_page ? ($countries_page->post_title ?: 'Страны') : 'Страны'];
    }

    $new_links[] = ['url' => get_permalink($country_id), 'text' => $country_title];

    $new_links[] = ['url' => $tour_list_url, 'text' => 'Туры'];

    $new_links[] = ['text' => get_the_title($tour_id)];
...
/**
 * Query var для страницы туров страны
 */
add_filter('query_vars', function ($vars) {
...
/**
 * Роут: /country/{country}/tours/ -> список туров страны
 * (ВАЖНО: single тура у тебя отдельно /country/{country}/tours/{tour}/)
 */
add_action('init', function () {
  add_rewrite_rule(
    '^country/([^/]+)/tours/?$',
    'index.php?post_type=country&name=$matches[1]&country_tours=$matches[1]',
    'top'
  );
}, 25);

/**
 * Роутинг на шаблон country-tours.php
 */
add_action('template_redirect', function () {
