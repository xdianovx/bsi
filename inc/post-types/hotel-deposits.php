<?php
/**
 * CPT: hotel_deposit (Депозиты в отелях)
 * По образцу hotel_info. Привязка к стране через ACF `hotel_deposit_country`.
 * URL раздела страны: /country/{country}/depozity/
 * (роут зарегистрирован в inc/post-types/country.php + single-country.php →
 *  шаблон country-deposits.php).
 *
 * Модель: одна запись на страну (шаблон берёт последнюю опубликованную).
 */
...
add_action('init', function () {
  register_post_type('hotel_deposit', [
    'labels' => [
      'name' => 'Депозиты в отелях',
      'singular_name' => 'Депозиты в отелях',
      'add_new' => 'Добавить',
...
add_action('acf/init', function () {
  ...
  acf_add_local_field_group([
    'key' => 'group_hotel_deposit_country',
    'title' => 'Депозиты в отелях — страна',
    'position' => 'acf_after_title',
    'menu_order' => 0,
    'fields' => [
      [
        'key' => 'field_hotel_deposit_country',
        'label' => 'Страна',
        'name' => 'hotel_deposit_country',
        'type' => 'post_object',
        'post_type' => ['country'],
        'return_format' => 'id',
        'ui' => 1,
 ...
 */
add_filter('acf/fields/post_object/query/key=field_hotel_deposit_country', function ($args) {
  $args['post_parent'] = 0;
  return $args;
}, 10, 1);

/**
 * Yoast breadcrumbs: Главная > Страны > {Страна} > Депозиты в отелях
 */
add_filter('wpseo_breadcrumb_links', function ($links) {
  $country_slug = get_query_var('country_deposits');
