...
class HotelsApiClient
{
  ...
  /**
   * Расчёт заезда — исключение из правила выше.
   *
   * `/quote` не читает готовый кэш, а считает: на холодной дате хаб идёт к
   * поставщику, и ответ занимает 3-4 секунды — столько же на повторный запрос
   * той же даты. Гость же перебирает даты и длительности туда-сюда, а каждый
   * такой перебор держит PHP-воркер все эти секунды.
   *
   * Десять минут — компромисс: цены столько живут спокойно, а «мест нет»
   * не залипает надолго. Меняется фильтром bsi_hotels_api_cache_ttl.
   */
  private const CACHE_QUOTE = 600;
