...
class HotelsApiClient
{
   ...
   *
   * Свой кеш тут стоял, пока повтор запроса к хабу стоил 3-4 секунды. Хаб это
   * починил 2026-09-11 (не сохранялся пустой ответ оператора, и каждый повтор
   * шёл к нему заново) — теперь повтор отдаётся за 0,003 с, и кешировать нечего.
   *
   * Возвращать свой слой нельзя: хаб сам знает, когда цена устарела, а
   * закешированный `prices_status: updating` никогда не станет `fresh` —
   * догрузка цен будет опрашивать хаб до упора.
   *
   * Откуда пришёл ответ, видно по полю `refreshed`: true — хаб только что
   * спросил оператора, false — отдал из своего кеша.
   */
  private const CACHE_QUOTE = 0;
