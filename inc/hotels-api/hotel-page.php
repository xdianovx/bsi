 ...
 *
 * Тот же URL, что у отелей, заведённых в WordPress. Хаб — источник истины по
 * отелям, поэтому он идёт первым: если отель там есть, показываем его карточку,
 * даже когда в WordPress лежит пост с тем же слагом. Пост остаётся запасным
 * вариантом — для отелей, которых в хабе нет.
 */

add_action('template_redirect', 'bsi_hotels_api_hotel_page', 5);
function bsi_hotels_api_hotel_page(): void
{
  if (!is_404() && !is_singular('hotel')) {
    return;
  }
  ...
  try {
    ...
    // Отеля в хабе нет или связь оборвалась — отдаём то, что было:
    // пост WordPress, а если его нет, то честный 404.
    return;
  }
