...
export const initPopularToursSlider = () => {
  ...
  const sliderEl = root.querySelector(".popular-tours-slider");
  const wrapper = root.querySelector(".popular-tours-slider .swiper-wrapper");
  const filter = root.querySelector(".popular-tours-filter");
  ...
  const load = async (countryId) => {
    ...
    try {
      ...
      body.set("action", "popular_tours_by_country");
