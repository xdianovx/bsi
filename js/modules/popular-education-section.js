...
export const initPopularEducationSlider = () => {
  const root = document.querySelector(".popular-education__section");
  if (!root) return;

  const sliderEl = root.querySelector(".popular-education-slider");
  const wrapper = root.querySelector(".popular-education-slider .swiper-wrapper");
  const filter = root.querySelector(".popular-education-filter");
  ...
  const swiperConfig = {
    ...
    observeParents: true,
    navigation: {
      nextEl: ".popular-education-arrow-next",
      prevEl: ".popular-education-arrow-prev",
    },
    breakpoints: {
      320: { slidesPerView: 1 },
      769: { slidesPerView: 3 },
  ...
  const load = async (countryId) => {
    ...
    try {
      ...
      body.set("action", "popular_education_by_country");
