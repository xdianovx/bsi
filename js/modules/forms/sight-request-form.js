...
/**
 * Заявка «Хочу сюда»: модалка #modal-sight-request.
 * Триггер — .js-sight-request-btn с data-sight-title / data-sight-country.
 * Обработчик — action `sight_request` (inc/requests/ajax-sight-request.php).
 */

function getModalRoot() {
  return document.getElementById("modal-sight-request");
}

function getModalForm() {
  const modal = getModalRoot();
  return modal ? modal.querySelector(".js-sight-request-form") : null;
}
...
function populateModal(data) {
  ...
  const titleEl = document.getElementById("modal-sight-request-title");
  if (titleEl) {
    titleEl.textContent = data.sightTitle ? `Хочу сюда: ${data.sightTitle}` : "Хочу сюда";
  }
  ...
  setVal(".js-form-sight-title", data.sightTitle || "");
  setVal(".js-form-sight-country", data.sightCountry || "");
...
async function submitForm(e) {
  ...
  try {
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        MicroModal.show("modal-sight-request-success", {
          awaitCloseAnimation: true,
          onClose: () => resetForm(),
        });

        setTimeout(() => {
          MicroModal.close("modal-sight-request-success");
        }, 2000);
      }, 300);
    } else {
    ...
    if (error.message === RECAPTCHA_NOT_LOADED) {
      ...
      console.error("Sight request submit error:", error);
...
export function initSightRequestForm() {
  const modal = getModalRoot();
  if (!modal) return;
  ...
  document.addEventListener("click", (e) => {
    const btn = e.target.closest(".js-sight-request-btn");
    if (!btn) return;
    e.preventDefault();

    resetForm();
    populateModal({
      sightTitle: btn.dataset.sightTitle || "",
      sightCountry: btn.dataset.sightCountry || "",
    });

    MicroModal.show("modal-sight-request", {
      disableScroll: true,
      disableFocus: false,
